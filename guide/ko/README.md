...
LLM 에이전트 워크플로우에서는 코드 탐색을 `search`로 시작하십시오. 식별자
검색과 "왜 이렇게 만들었는가" 평문 질문을 하나의 인덱스로 답하고, 모든 답에
그래프를 이어 걸을 `node_id`가 실려 옵니다. 생성 문서는 `get_doc_content`로
읽습니다. Top1 정답을 강제하기보다 작은 파일 후보로 빠르게 경로를 좁힌 뒤
graph 도구로 정확한 위치와 관계를 확인하는 흐름을 권장합니다.

브라우저 Wiki는 `ccg-server`에서 `--wiki-dir`가 빌드된 React asset을
가리킬 때 제공됩니다. 표시는 graph database를 우선 사용하고,
`wiki-index.json`은 tree 호환 snapshot으로 사용합니다. runtime 문서 검색은
DB-backed graph/annotation evidence를 사용하며, 시각적 Graph 탭은
