...
| `/status`가 `degraded` | 마지막 웹훅 또는 사후 처리 상태 확인 필요 | 실패한 저장소/설정을 수정하고 새로운 push 또는 수동 업데이트로 재시도하십시오. |
...
| `/status`가 `degraded`임 | 마지막 웹훅 또는 사후 처리 실패 | 저장소 설정을 수정하거나 업데이트/사후 처리를 재실행하십시오. |
