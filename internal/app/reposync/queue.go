...
// Observability wraps queue process/attempt lifecycles without coupling app policy to tracing infrastructure.
// @intent allow tracing adapters to enrich contexts and logs while queue behavior remains infrastructure-free.
type Observability interface {
	Start(ctx context.Context, operation, repo, branch string) (context.Context, func())
	LogArgs(ctx context.Context) []any
}

// @intent preserve queue behavior when no observability adapter is configured.
type noopObservability struct{}

// @intent return the original context and a safe no-op lifecycle closer.
func (noopObservability) Start(ctx context.Context, _, _, _ string) (context.Context, func()) {
	return ctx, func() {}
}

// @intent contribute no structured trace fields when observability is disabled.
func (noopObservability) LogArgs(context.Context) []any { return nil }
...
func NewSyncQueueWithConfig(ctx context.Context, workers int, handler SyncHandlerFunc, cfg QueueConfig) *SyncQueue {
	...
	if q.observability == nil {
		q.observability = noopObservability{}
	}
...
func (q *SyncQueue) worker() {
	...
	for {
		...
		processCtx, endProcess := q.observability.Start(payload.ctx, "webhook.sync.queue.process", repo, payload.branch)

		slog.InfoContext(processCtx, "sync queue processing", append(q.observability.LogArgs(processCtx), "repo", repo)...)
...
func (q *SyncQueue) safeHandle(repo string, payload syncPayload) bool {
	...
	for attempt := 1; attempt <= cfg.MaxAttempts; attempt++ {
		...
		if IsNonRetryable(err) {
			slog.ErrorContext(payload.ctx, "sync handler failed with non-retryable error", append(q.observability.LogArgs(payload.ctx), "repo", repo, "error", err)...)
			q.recordFailure(repo, payload.branch, err)
			return false
		}

		if attempt == cfg.MaxAttempts {
			slog.ErrorContext(payload.ctx, "sync handler failed, giving up", append(q.observability.LogArgs(payload.ctx), "repo", repo, "attempts", attempt, "error", err)...)
			q.recordFailure(repo, payload.branch, err)
			return false
		}

		slog.WarnContext(payload.ctx, "sync handler failed, retrying", append(q.observability.LogArgs(payload.ctx), "repo", repo, "attempt", attempt, "retryIn", delay, "error", err)...)

		select {
		case <-q.ctx.Done():
			slog.WarnContext(payload.ctx, "sync retry cancelled", append(q.observability.LogArgs(payload.ctx), "repo", repo, "attempt", attempt)...)
			return false
		case <-payload.ctx.Done():
			slog.WarnContext(payload.ctx, "sync retry cancelled by payload context", append(q.observability.LogArgs(payload.ctx), "repo", repo, "attempt", attempt)...)
			return false
...
func (q *SyncQueue) tryHandle(repo string, payload syncPayload) (err error) {
	...
	ctx, endAttempt := q.observability.Start(ctx, "webhook.sync.attempt", repo, payload.branch)
	defer cancel()
	defer endAttempt()
	defer func() {
		if r := recover(); r != nil {
			slog.ErrorContext(ctx, "sync handler panicked", append(q.observability.LogArgs(ctx), "repo", repo, "panic", r)...)
