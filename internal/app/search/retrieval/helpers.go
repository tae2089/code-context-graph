...
var retrievableNodeKinds = []graph.NodeKind{
	graph.NodeKindFile,
...
func dbFieldName(kind graph.TagKind) string {
	if kind == graph.TagIndex {
...
func dbFieldWeight(kind graph.TagKind) int {
	switch kind {
	case graph.TagIntent:
		return 7
	case graph.TagIndex:
		return 6
	case graph.TagDomainRule, graph.TagRequires, graph.TagEnsures:
		return 5
	case graph.TagSideEffect, graph.TagMutates:
		return 4
	case graph.TagSee:
		return 3
	default:
...
func DBMatches(nodes []graph.Node, annotations map[uint]*graph.Annotation) []SearchResult {
	...
	matches := make([]SearchResult, 0, len(nodes))
