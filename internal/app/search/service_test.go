...
var sentenceHomes = map[string]bool{
	filepath.Join("internal", "app", "search", "service.go"):      true,
	filepath.Join("internal", "app", "search", "service_test.go"): true,
}
...
func assertSentenceWrittenOnce(t *testing.T) {
	...
	walkErr := filepath.WalkDir(root, func(path string, entry fs.DirEntry, err error) error {
		...
		if strings.Contains(string(content), OffsetMustNotBeNegative) {
	...
	if len(copies) > 0 {
		t.Fatalf("the sentence %q is written again in %v; name OffsetMustNotBeNegative instead so one reword reaches every surface", OffsetMustNotBeNegative, copies)
	}
}
...
func repositoryRootForSentenceScan(t *testing.T) string {
	...
	if !ok {
		t.Fatal("resolve the search service test file path")
	}
