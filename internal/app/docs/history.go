...
func LoadHistory(files StateFiles, path string) (*History, error) {
	h := &History{Entries: map[string]int{}}

	data, exists, err := files.ReadPath(path)
	if err != nil {
		return nil, err
	}
	if !exists {
...
func (h *History) Save(files StateFiles, path string) error {
	data, err := json.MarshalIndent(h, "", "  ")
	if err != nil {
		return err
	}
	return files.WritePath(path, data)
}
...
func WriteYamlRules(files StateFiles, cfgPath string, triggered []string) error {
	data, _, err := files.ReadPath(cfgPath)
	if err != nil {
		return err
	}
	...
	return files.WritePath(cfgPath, []byte(out.String()))
}
