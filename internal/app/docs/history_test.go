...
func TestHistory_FirstRun_AllCountsOne(t *testing.T) {
	...
	h, err := LoadHistory(testStateFiles, histPath)
	if err != nil {
	...
	// Persist and re-read
	if err := h.Save(testStateFiles, histPath); err != nil {
		t.Fatalf("save: %v", err)
	}

	h2, err := LoadHistory(testStateFiles, histPath)
	if err != nil {
...
func TestHistory_SecondRun_TwiceRuleTriggered(t *testing.T) {
	...
	// First run
	h1, _ := LoadHistory(testStateFiles, histPath)
	h1.Update([]string{"unannotated:pkg/a.go::Foo", "missing:pkg/b.go"})
	h1.Save(testStateFiles, histPath)

	// Second run — same items (plus one new, one resolved)
	h2, _ := LoadHistory(testStateFiles, histPath)
...
func TestHistory_ResolvedItem_Removed(t *testing.T) {
	...
	h, _ := LoadHistory(testStateFiles, histPath)
	h.Update([]string{"unannotated:pkg/a.go::Foo"})
	h.Save(testStateFiles, histPath)
...
func TestWriteYamlRules_AddsNewRules(t *testing.T) {
	...
	if err := WriteYamlRules(testStateFiles, cfgPath, triggered); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestWriteYamlRules_Idempotent(t *testing.T) {
	...
	// Write twice
	WriteYamlRules(testStateFiles, cfgPath, triggered)
	WriteYamlRules(testStateFiles, cfgPath, triggered)
