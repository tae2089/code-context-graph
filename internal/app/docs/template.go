...
func groupByFile(nodes []graph.Node, annByID map[uint]*graph.Annotation, edgesByFromID map[uint][]graph.Edge) []nodeGroup {
	fileAnns := map[string]*graph.Annotation{}
	fileNodeMap := map[string][]graph.Node{}
...
func (g *Generator) writeFileDoc(grp nodeGroup) error {
	...
	return g.Files.Write(grp.FilePath+".md", []byte(generatorManagedMarker+content))
}
...
func (g *Generator) writeIndex(groups []nodeGroup) error {
	...
	return g.Files.Write("index.md", []byte(generatorManagedMarker+content))
}
...
func renderFileDoc(grp nodeGroup) string {
	...
	if grp.FileAnn != nil {
		if idx := tagValue(grp.FileAnn, graph.TagIndex); idx != "" {
			fmt.Fprintf(&b, "\n> %s\n", idx)
		}
	}
	...
	kindTitle := map[graph.NodeKind]string{
		graph.NodeKindFunction: "Functions",
		graph.NodeKindClass:    "Classes",
		graph.NodeKindType:     "Types",
		graph.NodeKindTest:     "Tests",
	}
...
func renderSymbol(b *strings.Builder, n graph.Node, ann *graph.Annotation, edges []graph.Edge) {
	fmt.Fprintf(b, "\n### %s\n", n.Name)
	...
	if v := tagValue(ann, graph.TagIntent); v != "" {
		fmt.Fprintf(b, "- **Intent:** %s\n", v)
	}
	if rules := tagValues(ann, graph.TagDomainRule); len(rules) > 0 {
		fmt.Fprintf(b, "- **Domain Rules:**\n")
		for _, r := range rules {
			fmt.Fprintf(b, "  - %s\n", r)
		}
	}
	if effects := tagValues(ann, graph.TagSideEffect); len(effects) > 0 {
		fmt.Fprintf(b, "- **Side Effects:** %s\n", strings.Join(effects, "; "))
	}
	if mutates := tagValues(ann, graph.TagMutates); len(mutates) > 0 {
		fmt.Fprintf(b, "- **Mutates:** %s\n", strings.Join(mutates, "; "))
	}
	if reqs := tagValues(ann, graph.TagRequires); len(reqs) > 0 {
		fmt.Fprintf(b, "- **Requires:**\n")
		for _, r := range reqs {
			fmt.Fprintf(b, "  - %s\n", r)
		}
	}
	if ensures := tagValues(ann, graph.TagEnsures); len(ensures) > 0 {
		fmt.Fprintf(b, "- **Ensures:**\n")
		for _, e := range ensures {
			fmt.Fprintf(b, "  - %s\n", e)
		}
	}
	if params := tagsWithName(ann, graph.TagParam); len(params) > 0 {
		fmt.Fprintf(b, "- **Params:**\n")
		for _, p := range params {
			fmt.Fprintf(b, "  - `%s` \u2014 %s\n", p.Name, p.Value)
		}
	}
	if v := tagValue(ann, graph.TagReturn); v != "" {
		fmt.Fprintf(b, "- **Returns:** %s\n", v)
	}
	if sees := tagValues(ann, graph.TagSee); len(sees) > 0 {
		fmt.Fprintf(b, "- **See:** %s\n", strings.Join(sees, ", "))
	}
...
func renderIndex(groups []nodeGroup) string {
	...
	for _, grp := range groups {
		desc := "\u2014"
		if grp.FileAnn != nil {
			if v := tagValue(grp.FileAnn, graph.TagIndex); v != "" {
				desc = v
			}
		}
...
func kindIdx(order []graph.NodeKind, k graph.NodeKind) int {
	for i, o := range order {
		if o == k {
			return i
		}
...
func tagValue(ann *graph.Annotation, kind graph.TagKind) string {
...
func tagValues(ann *graph.Annotation, kind graph.TagKind) []string {
	var vals []string
	for _, t := range ann.Tags {
		if t.Kind == kind {
...
func tagsWithName(ann *graph.Annotation, kind graph.TagKind) []graph.DocTag {
