// Change-risk application characterization tests.
package changes
...
func TestAnalyzePage_AppliesLimitOffsetAndHasMore(t *testing.T) {
	...
	svc := New(graphgorm.New(db), &mockGit{files: files, hunks: hunks})

	page1, err := svc.AnalyzePage(context.Background(), ".", "main", 2, 0)
	if err != nil {
	...
	page2, err := svc.AnalyzePage(context.Background(), ".", "main", 2, 2)
	if err != nil {
	...
	page3, err := svc.AnalyzePage(context.Background(), ".", "main", 2, 4)
	if err != nil {
...
func TestAnalyzePage_PreservesAnalyzeOrderingForPagedConsumers(t *testing.T) {
	...
	page, err := svc.AnalyzePage(context.Background(), ".", "main", 2, 1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAnalyzePage_MatchesAnalyzeAcrossRepresentativeWindows(t *testing.T) {
	...
	for _, req := range []struct{ limit, offset int }{{limit: 1, offset: 0}, {limit: 3, offset: 0}, {limit: 3, offset: 2}, {limit: 2, offset: 5}} {
		page, err := svc.AnalyzePage(context.Background(), ".", "main", req.limit, req.offset)
		if err != nil {
		...
		if page.HasMore != wantHasMore {
			t.Fatalf("AnalyzePage(%+v) has_more=%v, want %v", req, page.HasMore, wantHasMore)
		}
	}
}
...
func TestAnalyzePage_RejectsLimitAboveMax(t *testing.T) {
	...
	if _, err := svc.AnalyzePage(context.Background(), ".", "main", maxAnalyzePageLimit+1, 0); err == nil {
...
func TestAnalyzePage_OffsetBeyondTotalReturnsEmpty(t *testing.T) {
	...
	page, err := svc.AnalyzePage(context.Background(), ".", "main", 10, 50)
	if err != nil {
		t.Fatal(err)
	}
	if len(page.Items) != 0 {
...
func analyzeAll(svc *Service, ctx context.Context) ([]RiskEntry, error) {
	page, err := svc.AnalyzePage(ctx, ".", "main", 500, 0)
