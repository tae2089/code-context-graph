	...
	return []server.ServerTool{
		...
		},
		{
			Tool: mcp.NewTool("get_community", withNamespaceParam(
				...
				mcp.WithNumber("member_limit", mcp.Description("Maximum number of members to return when include_members=true (default: 100)")),
				mcp.WithNumber("member_offset", mcp.Description("Zero-based member offset when include_members=true (default: 0)")),
			)...),
		...
		},
		{
			Tool: mcp.NewTool("get_architecture_overview", withNamespaceParam(
				...
				mcp.WithNumber("community_limit", mcp.Description("Maximum number of communities to return (default: 50)")),
				mcp.WithNumber("community_offset", mcp.Description("Zero-based community offset for pagination (default: 0)")),
				mcp.WithNumber("coupling_limit", mcp.Description("Maximum number of coupling pairs to return (default: 50)")),
				mcp.WithNumber("coupling_offset", mcp.Description("Zero-based coupling offset for pagination (default: 0)")),
			)...),
