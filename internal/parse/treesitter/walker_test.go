...
func TestParseCSharp_Method(t *testing.T) {
	src := `class Foo {
    void Bar() {}
}
`
	w := NewWalker(CSharpSpec)
	nodes, _, err := w.Parse("Foo.cs", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	funcNodes := filterByKind(nodes, model.NodeKindFunction)
...
func TestParseCSharp_Class(t *testing.T) {
	src := `class Foo {
}
`
	w := NewWalker(CSharpSpec)
...
func TestParseCSharp_Interface(t *testing.T) {
	src := `interface IFoo {
    void Bar();
}
`
	w := NewWalker(CSharpSpec)
	nodes, _, err := w.Parse("IFoo.cs", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	typeNodes := filterByKind(nodes, model.NodeKindType)
...
func TestParseCSharp_Using(t *testing.T) {
	src := `using System;
`
	w := NewWalker(CSharpSpec)
	_, edges, err := w.Parse("Foo.cs", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	importEdges := filterEdgesByKind(edges, model.EdgeKindImportsFrom)
...
func TestParseCSharp_Call(t *testing.T) {
	src := `class Foo {
    void Bar() {
        Baz();
    }
...
func TestParseCSharp_Namespace(t *testing.T) {
	src := `namespace Ns {
    class Foo {}
}
`
...
func TestParseCSharp_TestAttribute(t *testing.T) {
	src := `class FooTest {
    [Test]
    void TestFoo() {}
}
`
	w := NewWalker(CSharpSpec)
	nodes, _, err := w.Parse("FooTest.cs", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	testNodes := filterByKind(nodes, model.NodeKindTest)
...
func TestParseSwift_Function(t *testing.T) {
	src := `func foo() {
}
`
	w := NewWalker(SwiftSpec)
	nodes, _, err := w.Parse("app.swift", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	funcNodes := filterByKind(nodes, model.NodeKindFunction)
...
func TestParseSwift_Class(t *testing.T) {
	src := `class Foo {
}
`
	w := NewWalker(SwiftSpec)
...
func TestParseSwift_Struct(t *testing.T) {
	src := `struct Bar {
}
`
	w := NewWalker(SwiftSpec)
...
func TestParseSwift_Protocol(t *testing.T) {
	src := `protocol Baz {
}
`
	w := NewWalker(SwiftSpec)
...
func TestParseSwift_Import(t *testing.T) {
	src := `import Foundation
`
	w := NewWalker(SwiftSpec)
	_, edges, err := w.Parse("app.swift", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	importEdges := filterEdgesByKind(edges, model.EdgeKindImportsFrom)
...
func TestParseSwift_Call(t *testing.T) {
	src := `func bar() {}
func main() {
    bar()
}
...
func TestParseSwift_Extension(t *testing.T) {
	src := `class Foo {}
extension Foo {
    func bar() {}
}
`
	w := NewWalker(SwiftSpec)
	nodes, edges, err := w.Parse("app.swift", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	funcNodes := filterByKind(nodes, model.NodeKindFunction)
...
func TestParseScala_Function(t *testing.T) {
	src := `def foo(): Unit = {
}
`
	w := NewWalker(ScalaSpec)
	nodes, _, err := w.Parse("app.scala", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	funcNodes := filterByKind(nodes, model.NodeKindFunction)
...
func TestParseScala_Class(t *testing.T) {
	src := `class Foo {
}
`
	w := NewWalker(ScalaSpec)
...
func TestParseScala_Object(t *testing.T) {
	src := `object Foo {
}
`
	w := NewWalker(ScalaSpec)
...
func TestParseScala_Trait(t *testing.T) {
	src := `trait Baz {
}
`
	w := NewWalker(ScalaSpec)
...
func TestParseScala_Import(t *testing.T) {
	src := `import scala.io._
`
	w := NewWalker(ScalaSpec)
	_, edges, err := w.Parse("app.scala", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	importEdges := filterEdgesByKind(edges, model.EdgeKindImportsFrom)
...
func TestParseScala_Call(t *testing.T) {
	src := `object Main {
  def main(): Unit = {
    foo()
  }
...
func TestParseBash_Function(t *testing.T) {
	src := `foo() {
    echo "hello"
}
`
	w := NewWalker(BashSpec)
	nodes, _, err := w.Parse("app.sh", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	funcNodes := filterByKind(nodes, model.NodeKindFunction)
...
func TestParseBash_FunctionKeyword(t *testing.T) {
	src := `function bar() {
    echo "hello"
}
`
...
func TestParseBash_Call(t *testing.T) {
	src := `foo() {
    echo "hello"
}
foo
`
	w := NewWalker(BashSpec)
	_, edges, err := w.Parse("app.sh", []byte(src))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	callEdges := filterEdgesByKind(edges, model.EdgeKindCalls)
...
func TestParseBash_Source(t *testing.T) {
	src := `source ./lib.sh
`
	w := NewWalker(BashSpec)
	_, edges, err := w.Parse("app.sh", []byte(src))
	...
	// Bash source는 command이므로 import로 감지됨
	importEdges := filterEdgesByKind(edges, model.EdgeKindImportsFrom)
	if len(importEdges) == 0 {
		t.Fatal("expected at least 1 IMPORTS_FROM edge for source, got 0")
	}
}
...
func TestWalker_AttributeTest_CSharp(t *testing.T) {
...
func TestWalker_Extension_Swift(t *testing.T) {
	src := `class Foo {}
extension Foo {
    func bar() {}
}
...
func TestE2E_ParseMultiLangProject(t *testing.T) {
	...
	files := map[string]struct {
		...
		"app.cs":   {CSharpSpec, "class App { void Run() {} }\n"},
	}
	...
	expectedLangs := []string{"go", "python", "javascript", "rust", "csharp"}
