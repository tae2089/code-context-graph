...
func mergeLintRulesWithAutoRules(stateDir string) ([]lintRule, error) {
	autoRules, err := docs.LoadAutoRules(contentfiles.NewStateFiles(), filepath.Join(stateDir, "auto-rules.yaml"))
	if err != nil {
		return nil, err
	}
...
func migrateAutoRulesFromConfig(cfgPath, stateDir string) (int, error) {
	...
	if err := autoRules.Save(contentfiles.NewStateFiles(), filepath.Join(stateDir, "auto-rules.yaml")); err != nil {
		return 0, err
	}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Twice Rule: compare with previous run history
			{
				...
				history, err := docs.LoadHistory(contentfiles.NewStateFiles(), histPath)
				if err != nil {
					...
					if saveErr := history.Save(contentfiles.NewStateFiles(), histPath); saveErr != nil {
						deps.Logger.Warn("save lint history failed", "error", saveErr)
					}

					if len(triggered) > 0 {
						autoRulesPath := filepath.Join(hDir, "auto-rules.yaml")
						autoRules, loadErr := docs.LoadAutoRules(contentfiles.NewStateFiles(), autoRulesPath)
						if loadErr != nil {
							deps.Logger.Warn("load auto rules failed", "error", loadErr)
						} else {
							added := autoRules.Upsert(triggered)
							if saveErr := autoRules.Save(contentfiles.NewStateFiles(), autoRulesPath); saveErr != nil {
