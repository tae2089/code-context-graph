...
`)
	...
	foundFunc := false
	for _, n := range nodes {
		if n.Kind == graph.NodeKindFunction && n.Name == "Hello" {
			foundFunc = true
		}
	}
	if !foundFunc {
	...
	var count int64
	db.Model(&graph.Node{}).Where("file_path LIKE ?", "%hello.go").Count(&count)
	if count == 0 {
...
`)
	...
	var count int64
	db.Model(&graph.Node{}).Count(&count)
	if count == 0 {
...
`)
	...
	var count int64
	if db.Model(&graph.Node{}).Count(&count); count != 0 {
