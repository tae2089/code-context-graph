...
import (
	...
	storesearch "github.com/tae2089/code-context-graph/internal/adapters/outbound/searchsql"
...
func setupUpdateTest(t *testing.T) (*Deps, *bytes.Buffer, *bytes.Buffer, *gorm.DB) {
	...
	deps.Store = st
	deps.Walkers = map[string]ingest.Parser{".go": walker}
	deps.Syncer = incremental.New(st, walker)
	deps.UnitOfWork = storesearch.NewIngestUnitOfWork(db, sb, deps.Logger)
	deps.Search = storesearch.NewSearchWriter(db, sb, deps.Logger)
	deps.SearchReader = storesearch.NewReader(db, sb)
...
func setupUpdateGraphOnlyTest(t *testing.T) (*Deps, *bytes.Buffer, *bytes.Buffer, *gorm.DB) {
	...
	deps.Walkers = map[string]ingest.Parser{".go": walker}
	deps.Syncer = incremental.New(st, walker)
	deps.UnitOfWork = storesearch.NewIngestUnitOfWork(db, nil, deps.Logger)
	deps.Search = storesearch.NewSearchWriter(db, nil, deps.Logger)
...
`)
	...
	db.Model(&graph.Node{}).Count(&countBefore)
	if countBefore == 0 {
...
`)
	...
	db.Model(&graph.Node{}).Count(&countAfter)
...
`)
	...
	var before int64
	if err := db.Model(&graph.Node{}).Where("file_path = ?", "b.go").Count(&before).Error; err != nil {
	...
	var after int64
	if err := db.Model(&graph.Node{}).Where("file_path = ?", "b.go").Count(&after).Error; err != nil {
...
`)
	...
	if err := db.Model(&graph.Node{}).Where("file_path = ?", "ignored.gen.go").Count(&ignoredCount).Error; err != nil {
...
`)
	...
	nodes, err := deps.SearchReader.Query(context.Background(), "SearchableUpdated", 10)
	if err != nil {
...
func TestUpdateCommand_NamespaceSearchIsolation(t *testing.T) {
	...
	legacyNode := graph.Node{Namespace: "other", QualifiedName: "other.Legacy", Kind: graph.NodeKindFunction, Name: "Legacy", FilePath: "legacy.go", StartLine: 1, EndLine: 2, Language: "go"}
	if err := db.Create(&legacyNode).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&graph.SearchDocument{Namespace: "other", NodeID: legacyNode.ID, Content: "legacy symbol", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	sb := storesearch.NewSQLiteBackend()
	if err := sb.Rebuild(requestctx.WithNamespace(context.Background(), "other"), db); err != nil {
		t.Fatal(err)
	}
...
`)
	...
	otherResults, err := sb.Query(requestctx.WithNamespace(context.Background(), "other"), db, "legacy", 10)
	if err != nil {
		t.Fatal(err)
	}
