...
func TestSearchDocs_DBFallbackSucceedsWithoutDocIndex(t *testing.T) {
	...
	tmpDir := t.TempDir()
	deps.Runtime.RagIndexDir = filepath.Join(tmpDir, ".ccg")
	seedRetrieveDocsDBFallbackNode(t, deps, requestctx.DefaultNamespace, "billing.Processor", "Processor", "internal/billing/processor.go", graph.TagIntent, "payment settlement workflow", "payment settlement workflow")
	rebuildRetrieveDocsSearchBackend(t, deps, requestctx.DefaultNamespace)
...
func TestSearchDocs_DBFallbackIsNamespaceIsolated(t *testing.T) {
	...
	alpha := "alpha-service"
	beta := "beta-service"
	seedRetrieveDocsDBFallbackNode(t, deps, alpha, "alpha.Checkout", "Checkout", "checkout.go", graph.TagIntent, "sharedtenant alpha checkout", "sharedtenant alpha checkout")
	seedRetrieveDocsDBFallbackNode(t, deps, beta, "beta.Checkout", "Checkout", "checkout.go", graph.TagIntent, "sharedtenant beta checkout", "sharedtenant beta checkout")
...
func TestSearchDocs_DBFallbackDomainRuleUsesContextNamespace(t *testing.T) {
	...
	alpha := "alpha-service"
	beta := "beta-service"
	seedRetrieveDocsDBFallbackNode(t, deps, alpha, "alpha.Guard", "Guard", "guard.go", graph.TagDomainRule, "breakglass alpha approval", "breakglass alpha approval")
	seedRetrieveDocsDBFallbackNode(t, deps, beta, "beta.Guard", "Guard", "guard.go", graph.TagDomainRule, "breakglass beta approval", "breakglass beta approval")
	rebuildRetrieveDocsSearchBackend(t, deps, alpha)
	rebuildRetrieveDocsSearchBackend(t, deps, beta)
...
func TestSearchDocs_DBFallbackAnnotationOnlyMatch(t *testing.T) {
	...
	tmpDir := t.TempDir()
	deps.Runtime.RagIndexDir = filepath.Join(tmpDir, ".ccg")
	seedRetrieveDocsDBFallbackNode(t, deps, requestctx.DefaultNamespace, "policy.Guard", "Guard", "internal/policy/guard.go", graph.TagDomainRule, "breakglass approval required", "policy guard searchable")
	rebuildRetrieveDocsSearchBackend(t, deps, requestctx.DefaultNamespace)
...
func TestSearchDocs_DBFallbackHonorsLimit(t *testing.T) {
	...
	tmpDir := t.TempDir()
	deps.Runtime.RagIndexDir = filepath.Join(tmpDir, ".ccg")
	seedRetrieveDocsDBFallbackNode(t, deps, requestctx.DefaultNamespace, "alpha.First", "First", "internal/alpha/file.go", graph.TagIntent, "groupterm first", "groupterm first")
	seedRetrieveDocsDBFallbackNode(t, deps, requestctx.DefaultNamespace, "alpha.Second", "Second", "internal/alpha/file.go", graph.TagIntent, "groupterm second", "groupterm second")
	seedRetrieveDocsDBFallbackNode(t, deps, requestctx.DefaultNamespace, "beta.Only", "Only", "internal/beta/file.go", graph.TagIntent, "groupterm beta", "groupterm beta")
	rebuildRetrieveDocsSearchBackend(t, deps, requestctx.DefaultNamespace)
...
func TestSearchDocs_DBFallbackResponseShapeStable(t *testing.T) {
	...
	tmpDir := t.TempDir()
	deps.Runtime.RagIndexDir = filepath.Join(tmpDir, ".ccg")
	seedRetrieveDocsDBFallbackNode(t, deps, requestctx.DefaultNamespace, "stable.Shape", "Shape", "internal/stable/shape.go", graph.TagIntent, "stable shape", "stable shape")
	rebuildRetrieveDocsSearchBackend(t, deps, requestctx.DefaultNamespace)
...
func TestSearchDocs_UsesDBWhenDocIndexFileExists(t *testing.T) {
	...
	tmpDir := t.TempDir()
	deps.Runtime.RagIndexDir = filepath.Join(tmpDir, ".ccg")
	seedRetrieveDocsDBFallbackNode(t, deps, requestctx.DefaultNamespace, "db.Only", "Only", "internal/db/only.go", graph.TagIntent, "precedence db", "precedence db")
	rebuildRetrieveDocsSearchBackend(t, deps, requestctx.DefaultNamespace)

	if err := os.MkdirAll(deps.Runtime.RagIndexDir, 0o755); err != nil {
		t.Fatal(err)
	}
	idx := &wiki.Index{Root: &wiki.TreeNode{ID: "root", Label: "Root", Kind: "root", Children: []*wiki.TreeNode{{ID: "community:json", Label: "JSON Wins", Kind: "community", Summary: "json index", Children: []*wiki.TreeNode{}}}}}
	idxBytes, err := json.Marshal(idx)
	if err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(deps.Runtime.RagIndexDir, "doc-index.json"), idxBytes, 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestSearchDocs_RejectsInvalidNamespace(t *testing.T) {
	...
	var response []wiki.SearchResult
...
func TestSearchDocs_WithNamespace(t *testing.T) {
	...
	tmpDir := t.TempDir()
	deps.Runtime.RagIndexDir = filepath.Join(tmpDir, ".ccg")
	comm := graph.Community{Key: "auth", Label: "Auth", Description: "authentication service"}
	if err := testDBFor(deps).Create(&comm).Error; err != nil {
	...
	if err := testDBFor(deps).Create(&authNode).Error; err != nil {
		t.Fatalf("create auth node: %v", err)
	}
	if err := testDBFor(deps).Create(&graph.CommunityMembership{CommunityID: comm.ID, NodeID: authNode.ID}).Error; err != nil {
		t.Fatalf("create auth membership: %v", err)
	}
	authAnn := graph.Annotation{NodeID: authNode.ID}
	if err := testDBFor(deps).Create(&authAnn).Error; err != nil {
		t.Fatalf("create auth annotation: %v", err)
	}
	if err := testDBFor(deps).Create(&graph.DocTag{AnnotationID: authAnn.ID, Kind: graph.TagIntent, Value: "auth check", Ordinal: 0}).Error; err != nil {
...
func seedRetrieveDocsDBFallbackNode(t *testing.T, deps *Deps, namespace, qualifiedName, name, filePath string, tagKind graph.TagKind, tagValue, searchContent string) graph.Node {
	t.Helper()
	node := graph.Node{
		Namespace:     namespace,
	...
	if err := testDBFor(deps).Create(&node).Error; err != nil {
		t.Fatalf("create node %s: %v", qualifiedName, err)
	}
	ann := graph.Annotation{NodeID: node.ID, Summary: tagValue}
	if err := testDBFor(deps).Create(&ann).Error; err != nil {
		t.Fatalf("create annotation %s: %v", qualifiedName, err)
	}
	if err := testDBFor(deps).Create(&graph.DocTag{AnnotationID: ann.ID, Kind: tagKind, Value: tagValue, Ordinal: 0}).Error; err != nil {
		t.Fatalf("create doc tag %s: %v", qualifiedName, err)
	}
	if err := testDBFor(deps).Create(&graph.SearchDocument{Namespace: namespace, NodeID: node.ID, Content: searchContent, Language: "go"}).Error; err != nil {
		t.Fatalf("create search document %s: %v", qualifiedName, err)
	}
	return node
}

func seedRagTreeDBFallbackCommunity(t *testing.T, deps *Deps, namespace, communityKey, communityLabel, communityDescription, qualifiedName, name, filePath string) graph.Node {
	t.Helper()
	comm := graph.Community{Namespace: namespace, Key: communityKey, Label: communityLabel, Strategy: "test", Description: communityDescription}
	if err := testDBFor(deps).Create(&comm).Error; err != nil {
		t.Fatalf("create community %s: %v", communityKey, err)
	}
	node := seedRetrieveDocsDBFallbackNode(t, deps, namespace, qualifiedName, name, filePath, graph.TagIntent, communityDescription, communityDescription)
	if err := testDBFor(deps).Create(&graph.CommunityMembership{CommunityID: comm.ID, NodeID: node.ID}).Error; err != nil {
...
func decodeSearchDocsResults(t *testing.T, result *mcp.CallToolResult) []wiki.SearchResult {
	t.Helper()
	var results []wiki.SearchResult
	if err := json.Unmarshal([]byte(getTextContent(result)), &results); err != nil {
		t.Fatalf("unmarshal search_docs response: %v", err)
