...
`), 0644)
	...
	node, err := testGraphStoreFor(deps).GetNode(context.Background(), "main.Hello")
	if err != nil {
		t.Fatal(err)
	}
	if node == nil {
...
func TestParseProject_RejectsPathOutsideConfiguredRoot(t *testing.T) {
	deps := setupTestDeps(t)
	deps.Runtime.RepoRoot = t.TempDir()
	outside := t.TempDir()
...
func TestParseProject_AllowsNamespacePathWhenRepoRootIsAlsoConfigured(t *testing.T) {
	deps := setupTestDeps(t)
	deps.Runtime.RepoRoot = t.TempDir()
	deps.Runtime.NamespaceRoot = t.TempDir()
	namespaceDir := filepath.Join(deps.Runtime.NamespaceRoot, "sample-ns")
...
func TestParseProject_FailsClosedWithoutConfiguredRoot(t *testing.T) {
	deps := setupTestDeps(t)
	deps.Runtime.RepoRoot = ""
	deps.Runtime.NamespaceRoot = ""
	dir := t.TempDir()
...
func TestHandler_GetNode(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Foo", Kind: graph.NodeKindFunction, Name: "Foo", FilePath: "foo.go", StartLine: 1, EndLine: 5, Language: "go"},
	})
...
func TestHandler_GetImpactRadius(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: nodeA.ID, ToNodeID: nodeB.ID, Kind: graph.EdgeKindCalls, Fingerprint: "calls-a-b"},
	})
...
func TestHandler_GetImpactRadius_BoundsResults(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.A", Kind: graph.NodeKindFunction, Name: "A", FilePath: "a.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.B", Kind: graph.NodeKindFunction, Name: "B", FilePath: "b.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.C", Kind: graph.NodeKindFunction, Name: "C", FilePath: "c.go", StartLine: 1, EndLine: 5, Language: "go"},
	})
	a, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.A")
	b, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.B")
	c, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.C")
	testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{{FromNodeID: a.ID, ToNodeID: b.ID, Kind: graph.EdgeKindCalls, Fingerprint: "a-b"}, {FromNodeID: b.ID, ToNodeID: c.ID, Kind: graph.EdgeKindCalls, Fingerprint: "b-c"}})
...
func TestHandler_GetImpactRadius_LogsTraceFields(t *testing.T) {
	...
	deps.Runtime.Logger = slog.New(slog.NewTextHandler(&logBuf, &slog.HandlerOptions{Level: slog.LevelInfo}))
	...
	testGraphStoreFor(deps).UpsertNodes(context.Background(), []graph.Node{{QualifiedName: "pkg.A", Kind: graph.NodeKindFunction, Name: "A", FilePath: "a.go", StartLine: 1, EndLine: 5, Language: "go"}})
...
func TestHandler_Search_PathFilter_RespectsPathBoundary(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "internal/api/handler.go::Handle", Kind: graph.NodeKindFunction, Name: "Handle", FilePath: "internal/api/handler.go", StartLine: 1, EndLine: 10, Language: "go"},
		{QualifiedName: "internal/api2/handler.go::Handle2", Kind: graph.NodeKindFunction, Name: "Handle2", FilePath: "internal/api2/handler.go", StartLine: 1, EndLine: 10, Language: "go"},
	})
	apiNode, _ := testGraphStoreFor(deps).GetNode(ctx, "internal/api/handler.go::Handle")
	api2Node, _ := testGraphStoreFor(deps).GetNode(ctx, "internal/api2/handler.go::Handle2")

	testDBFor(deps).Create(&graph.SearchDocument{NodeID: apiNode.ID, Content: "handle api request", Language: "go"})
	testDBFor(deps).Create(&graph.SearchDocument{NodeID: api2Node.ID, Content: "handle api request", Language: "go"})
	testSearchBackendFor(deps).Rebuild(ctx, testDBFor(deps))
...
func TestHandler_Search_PathFilter_CapsInternalFetchLimit(t *testing.T) {
	...
	setTestSearchBackend(deps, backend)
...
func TestHandler_GetAnnotation(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Login", Kind: graph.NodeKindFunction, Name: "Login", FilePath: "login.go", StartLine: 1, EndLine: 10, Language: "go"},
	})
	node, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Login")
	...
	testGraphStoreFor(deps).UpsertAnnotation(ctx, &graph.Annotation{
		...
		Tags: []graph.DocTag{
			{Kind: graph.TagIntent, Value: "Authenticate user", Ordinal: 0},
		},
	})
...
func TestHandler_GetAnnotation_ExposesDocTagTypeField(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Fn", Kind: graph.NodeKindFunction, Name: "Fn", FilePath: "f.go", StartLine: 1, EndLine: 3, Language: "go"},
	})
	node, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Fn")

	testGraphStoreFor(deps).UpsertAnnotation(ctx, &graph.Annotation{
		NodeID:  node.ID,
		Summary: "s",
		Tags: []graph.DocTag{
			{Kind: graph.TagParam, Type: "string", Name: "id", Value: "identifier", Ordinal: 0},
		},
	})
...
func TestHandler_TraceFlow(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		...
		{QualifiedName: "pkg.Middle", Kind: graph.NodeKindFunction, Name: "Middle", FilePath: "mid.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.End", Kind: graph.NodeKindFunction, Name: "End", FilePath: "end.go", StartLine: 1, EndLine: 5, Language: "go"},
	})
	start, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Start")
	mid, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Middle")

	testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: start.ID, ToNodeID: mid.ID, Kind: graph.EdgeKindCalls, Fingerprint: "calls-s-m"},
		{FromNodeID: mid.ID, ToNodeID: mid.ID + 1, Kind: graph.EdgeKindCalls, Fingerprint: "calls-m-e"},
	})
...
func TestHandler_TraceFlow_BoundsMembers(t *testing.T) {
	...
	ctx := context.Background()
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Start2", Kind: graph.NodeKindFunction, Name: "Start2", FilePath: "s.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.Next2", Kind: graph.NodeKindFunction, Name: "Next2", FilePath: "n.go", StartLine: 1, EndLine: 5, Language: "go"},
	})
	s, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Start2")
	n, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Next2")
	testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{{FromNodeID: s.ID, ToNodeID: n.ID, Kind: graph.EdgeKindCalls, Fingerprint: "s-n"}})
...
func TestTraceFlow_ReturnsFallbackMetadata(t *testing.T) {
	...
	if err := testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.StartFallback", Kind: graph.NodeKindFunction, Name: "StartFallback", FilePath: "s.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.StrictNext", Kind: graph.NodeKindFunction, Name: "StrictNext", FilePath: "n.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.FallbackNext", Kind: graph.NodeKindFunction, Name: "FallbackNext", FilePath: "f.go", StartLine: 1, EndLine: 5, Language: "go"},
	}); err != nil {
		t.Fatal(err)
	}
	start, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.StartFallback")
	strict, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.StrictNext")
	fallback, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.FallbackNext")
	if err := testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: start.ID, ToNodeID: strict.ID, Kind: graph.EdgeKindCalls, Fingerprint: "strict-edge"},
		{FromNodeID: start.ID, ToNodeID: fallback.ID, Kind: graph.EdgeKindFallbackCalls, Fingerprint: "fallback-edge"},
	}); err != nil {
		t.Fatal(err)
	}
...
func TestTraceFlow_RespectsIncludeFallbackCalls(t *testing.T) {
	...
	ctx := context.Background()
	if err := testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.StrictStart", Kind: graph.NodeKindFunction, Name: "StrictStart", FilePath: "s.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.StrictOnly", Kind: graph.NodeKindFunction, Name: "StrictOnly", FilePath: "n.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.FallbackOnly", Kind: graph.NodeKindFunction, Name: "FallbackOnly", FilePath: "f.go", StartLine: 1, EndLine: 5, Language: "go"},
	}); err != nil {
		t.Fatal(err)
	}
	start, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.StrictStart")
	strict, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.StrictOnly")
	fallback, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.FallbackOnly")
	if err := testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: start.ID, ToNodeID: strict.ID, Kind: graph.EdgeKindCalls, Fingerprint: "strict-edge"},
		{FromNodeID: start.ID, ToNodeID: fallback.ID, Kind: graph.EdgeKindFallbackCalls, Fingerprint: "fallback-edge"},
	}); err != nil {
...
func TestTraceFlow_CacheKeyIncludesIncludeFallbackCalls(t *testing.T) {
	...
	ctx := context.Background()
	if err := testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.CacheTarget", Kind: graph.NodeKindFunction, Name: "CacheTarget", FilePath: "cache.go", StartLine: 1, EndLine: 5, Language: "go"},
	}); err != nil {
		t.Fatal(err)
	}

	tracer := &mockFlowTracer{}
	deps.Analysis.Flow = tracer
	deps.Runtime.Cache = NewCache(5 * time.Minute)
...
`)
	...
	node, err := testGraphStoreFor(deps).GetNode(context.Background(), "hello.Hello")
	if err != nil || node == nil {
...
func TestBuildOrUpdateGraph_Incremental(t *testing.T) {
	...
	deps.Build.Incremental = mockSync
...
`)
	...
	node, err := testGraphStoreFor(deps).GetNode(context.Background(), "api.Handler")
	if err != nil || node == nil {
		t.Fatal("expected node api.Handler to exist after incremental include_paths build")
	}

	otherNode, _ := testGraphStoreFor(deps).GetNode(context.Background(), "other.Other")
	if otherNode != nil {
...
func TestBuildOrUpdateGraph_IncrementalIncludePaths_DefaultsToReplace(t *testing.T) {
	...
	if err := testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "api.Handler", Kind: graph.NodeKindFunction, Name: "Handler", FilePath: filepath.Join("src", "api", "handler.go"), StartLine: 1, EndLine: 2, Language: "go"},
		{QualifiedName: "other.Other", Kind: graph.NodeKindFunction, Name: "Other", FilePath: filepath.Join("src", "other", "other.go"), StartLine: 1, EndLine: 2, Language: "go"},
	}); err != nil {
		t.Fatal(err)
	}
...
`)
	...
	node, err := testGraphStoreFor(deps).GetNode(context.Background(), "calc.Add")
	if err != nil || node == nil {
...
func TestBuildOrUpdateGraph_DegradedOnSearchDocumentRefreshFailure(t *testing.T) {
	...
	backend := &failSearchBackend{}
	setTestSearchBackend(deps, backend)
	deps.Build.Maintenance = failingRefreshMaintenance{err: errors.New("search document refresh boom")}
...
func TestQueryGraph_CallersOf(t *testing.T) {
	...
	mockQ := &mockQueryService{
		result: []graph.Node{
			{QualifiedName: "pkg.Caller", Kind: graph.NodeKindFunction, Name: "Caller", FilePath: "caller.go"},
		},
	}
	deps.Graph.Query = mockQ

	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Func", Kind: graph.NodeKindFunction, Name: "Func", FilePath: "func.go", StartLine: 1, EndLine: 5, Language: "go"},
	})
...
func TestQueryGraph_CalleesOf(t *testing.T) {
	...
	mockQ := &mockQueryService{
		result: []graph.Node{
			{QualifiedName: "pkg.Callee", Kind: graph.NodeKindFunction, Name: "Callee", FilePath: "callee.go"},
		},
	}
	deps.Graph.Query = mockQ
...
func TestQueryGraph_CalleesOf_ReturnsTentativeProvenance(t *testing.T) {
	...
	if err := testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Source", Kind: graph.NodeKindFunction, Name: "Source", FilePath: "source.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.Strict", Kind: graph.NodeKindFunction, Name: "Strict", FilePath: "strict.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.Fallback", Kind: graph.NodeKindFunction, Name: "Fallback", FilePath: "fallback.go", StartLine: 1, EndLine: 5, Language: "go"},
	}); err != nil {
		t.Fatal(err)
	}
	source, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Source")
	strict, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Strict")
	fallback, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Fallback")
	if err := testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: source.ID, ToNodeID: strict.ID, Kind: graph.EdgeKindCalls, FilePath: "pkg/source.go", Line: 11, Fingerprint: "source-strict"},
		{FromNodeID: source.ID, ToNodeID: fallback.ID, Kind: graph.EdgeKindFallbackCalls, FilePath: "pkg/fallback.go", Line: 22, Fingerprint: "source-fallback"},
	}); err != nil {
		t.Fatal(err)
	}
	deps.Graph.Query = query.New(graphgorm.New(testDBFor(deps)))
	...
	if byName["pkg.Strict"]["edge_kind"] != string(graph.EdgeKindCalls) {
	...
	if byName["pkg.Fallback"]["edge_kind"] != string(graph.EdgeKindFallbackCalls) {
...
func TestQueryGraph_CalleesOf_PaginationMetadata(t *testing.T) {
	...
	if err := testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Source", Kind: graph.NodeKindFunction, Name: "Source", FilePath: "source.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.C2", Kind: graph.NodeKindFunction, Name: "C2", FilePath: "a.go", StartLine: 20, EndLine: 25, Language: "go"},
		{QualifiedName: "pkg.C1", Kind: graph.NodeKindFunction, Name: "C1", FilePath: "a.go", StartLine: 10, EndLine: 15, Language: "go"},
		{QualifiedName: "pkg.C3", Kind: graph.NodeKindFunction, Name: "C3", FilePath: "b.go", StartLine: 5, EndLine: 10, Language: "go"},
	}); err != nil {
		t.Fatal(err)
	}
	source, err := testGraphStoreFor(deps).GetNode(ctx, "pkg.Source")
	if err != nil {
		t.Fatal(err)
	}
	c1, err := testGraphStoreFor(deps).GetNode(ctx, "pkg.C1")
	if err != nil {
		t.Fatal(err)
	}
	c2, err := testGraphStoreFor(deps).GetNode(ctx, "pkg.C2")
	if err != nil {
		t.Fatal(err)
	}
	c3, err := testGraphStoreFor(deps).GetNode(ctx, "pkg.C3")
	if err != nil {
		t.Fatal(err)
	}
	if err := testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: source.ID, ToNodeID: c2.ID, Kind: graph.EdgeKindCalls, Fingerprint: "c2"},
		{FromNodeID: source.ID, ToNodeID: c1.ID, Kind: graph.EdgeKindCalls, Fingerprint: "c1"},
		{FromNodeID: source.ID, ToNodeID: c3.ID, Kind: graph.EdgeKindCalls, Fingerprint: "c3"},
	}); err != nil {
		t.Fatal(err)
	}
...
func TestQueryGraph_CacheKeyIncludesFallbackFlag(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Func", Kind: graph.NodeKindFunction, Name: "Func", FilePath: "func.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.Caller", Kind: graph.NodeKindFunction, Name: "Caller", FilePath: "caller.go", StartLine: 10, EndLine: 20, Language: "go"},
	})
...
func TestQueryGraph_InheritorsOf(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Base", Kind: graph.NodeKindClass, Name: "Base", FilePath: "base.go", StartLine: 1, EndLine: 10, Language: "go"},
	})
...
func TestQueryGraph_TargetFallbackAutoSelectsSingleShortNameMatch(t *testing.T) {
	...
	ctx := context.Background()
	mockQ := &mockQueryService{
		result:      []graph.Node{{QualifiedName: "pkg.Caller", Kind: graph.NodeKindFunction, Name: "Caller", FilePath: "caller.go"}},
		matchResult: []query.CandidateMatch{{QualifiedName: "pkg.runPostprocess", Kind: graph.NodeKindFunction, FilePath: "query.go", StartLine: 10}},
	}
	deps.Graph.Query = mockQ
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{{QualifiedName: "pkg.runPostprocess", Kind: graph.NodeKindFunction, Name: "runPostprocess", FilePath: "query.go", StartLine: 10, EndLine: 20, Language: "go"}})
...
func TestQueryGraph_TargetFallbackReturnsAmbiguousCandidates(t *testing.T) {
	deps := setupTestDeps(t)
	deps.Graph.Query = &mockQueryService{result: []graph.Node{}, matchResult: []query.CandidateMatch{
		{QualifiedName: "pkg.runPostprocess", Kind: graph.NodeKindFunction, FilePath: "a.go", StartLine: 10},
		{QualifiedName: "other.runPostprocess", Kind: graph.NodeKindFunction, FilePath: "b.go", StartLine: 5},
	}}
...
func TestListGraphStats_ReturnsAllCounts(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Func1", Kind: graph.NodeKindFunction, Name: "Func1", FilePath: "a.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.Func2", Kind: graph.NodeKindFunction, Name: "Func2", FilePath: "a.go", StartLine: 10, EndLine: 20, Language: "go"},
		{QualifiedName: "pkg.Class1", Kind: graph.NodeKindClass, Name: "Class1", FilePath: "b.py", StartLine: 1, EndLine: 30, Language: "python"},
	})
	n1, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Func1")
	n2, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.Func2")
	testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: n1.ID, ToNodeID: n2.ID, Kind: graph.EdgeKindCalls, Fingerprint: "c-f1-f2"},
	})
...
func TestDetectChanges_ReturnsRiskEntries(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.Changed", Kind: graph.NodeKindFunction, Name: "Changed", FilePath: "changed.go", StartLine: 1, EndLine: 20, Language: "go"},
	})

	setTestChangesClient(deps, &mockGitClient{
		changedFiles: []string{"changed.go"},
		hunks:        []changes.Hunk{{FilePath: "changed.go", StartLine: 5, EndLine: 15}},
	})
	repoRoot := t.TempDir()
	deps.Runtime.RepoRoot = repoRoot
...
func TestDetectChanges_DefaultBase(t *testing.T) {
	...
	setTestChangesClient(deps, mock)
	repoRoot := t.TempDir()
	deps.Runtime.RepoRoot = repoRoot
...
func TestGetAffectedFlows_ReturnsFlows(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.FnA", Kind: graph.NodeKindFunction, Name: "FnA", FilePath: "a.go", StartLine: 1, EndLine: 10, Language: "go"},
	})
	nodeA, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.FnA")

	// Create a flow.
	testDBFor(deps).Create(&graph.Flow{Name: "login-flow", Description: "Login flow"})
	var flow graph.Flow
	testDBFor(deps).First(&flow)
	testDBFor(deps).Create(&graph.FlowMembership{FlowID: flow.ID, NodeID: nodeA.ID, Ordinal: 0})
...
func TestGetAffectedFlows_RespectsNamespace(t *testing.T) {
	deps := setupTestDeps(t)
	ctx := requestctx.WithNamespace(context.Background(), "alpha")

	if err := testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{{QualifiedName: "pkg.FnA", Kind: graph.NodeKindFunction, Name: "FnA", FilePath: "a.go", StartLine: 1, EndLine: 10, Language: "go"}}); err != nil {
		t.Fatal(err)
	}
	nodeA, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.FnA")
	testDBFor(deps).Create(&graph.Flow{Namespace: "alpha", Name: "alpha-flow"})
	testDBFor(deps).Create(&graph.Flow{Namespace: "beta", Name: "beta-flow"})
	var alphaFlow, betaFlow graph.Flow
	testDBFor(deps).First(&alphaFlow, "name = ?", "alpha-flow")
	testDBFor(deps).First(&betaFlow, "name = ?", "beta-flow")
	testDBFor(deps).Create(&graph.FlowMembership{Namespace: "alpha", FlowID: alphaFlow.ID, NodeID: nodeA.ID, Ordinal: 0})
	testDBFor(deps).Create(&graph.FlowMembership{Namespace: "beta", FlowID: betaFlow.ID, NodeID: nodeA.ID, Ordinal: 0})

	setTestChangesClient(deps, &mockGitClient{changedFiles: []string{"a.go"}, hunks: []changes.Hunk{{FilePath: "a.go", StartLine: 1, EndLine: 10}}})
	repoRoot := t.TempDir()
	deps.Runtime.RepoRoot = repoRoot
...
func TestGetAffectedFlows_NoFlows(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.FnB", Kind: graph.NodeKindFunction, Name: "FnB", FilePath: "b.go", StartLine: 1, EndLine: 10, Language: "go"},
	})
...
func TestListFlows_SortByName(t *testing.T) {
	...
	testDBFor(deps).Create(&graph.Flow{Name: "beta-flow"})
	testDBFor(deps).Create(&graph.Flow{Name: "alpha-flow"})
...
func TestListFlows_SortByNodeCount(t *testing.T) {
	...
	testDBFor(deps).Create(&graph.Flow{Name: "small-flow"})
	testDBFor(deps).Create(&graph.Flow{Name: "big-flow"})
	var small, big graph.Flow
	testDBFor(deps).First(&small, "name = ?", "small-flow")
	testDBFor(deps).First(&big, "name = ?", "big-flow")

	testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{
		{QualifiedName: "pkg.N1", Kind: graph.NodeKindFunction, Name: "N1", FilePath: "n.go", StartLine: 1, EndLine: 5, Language: "go"},
		{QualifiedName: "pkg.N2", Kind: graph.NodeKindFunction, Name: "N2", FilePath: "n.go", StartLine: 10, EndLine: 15, Language: "go"},
		{QualifiedName: "pkg.N3", Kind: graph.NodeKindFunction, Name: "N3", FilePath: "n.go", StartLine: 20, EndLine: 25, Language: "go"},
	})
	n1, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.N1")
	n2, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.N2")
	n3, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.N3")

	testDBFor(deps).Create(&graph.FlowMembership{FlowID: small.ID, NodeID: n1.ID, Ordinal: 0})
	testDBFor(deps).Create(&graph.FlowMembership{FlowID: big.ID, NodeID: n2.ID, Ordinal: 0})
	testDBFor(deps).Create(&graph.FlowMembership{FlowID: big.ID, NodeID: n3.ID, Ordinal: 1})
...
func TestListFlows_Limit(t *testing.T) {
	...
	testDBFor(deps).Create(&graph.Flow{Name: "flow-1"})
	testDBFor(deps).Create(&graph.Flow{Name: "flow-2"})
	testDBFor(deps).Create(&graph.Flow{Name: "flow-3"})
...
func TestBuildOrUpdateGraph_Incremental_ExistingFilesNamespaceScoped(t *testing.T) {
	...
	testGraphStoreFor(deps).UpsertNodes(ctxA, []graph.Node{
		{QualifiedName: "a.F", Kind: graph.NodeKindFunction, Name: "F", FilePath: "/ns-a-only/file.go", StartLine: 1, EndLine: 2, Language: "go"},
	})
	testGraphStoreFor(deps).UpsertNodes(ctxB, []graph.Node{
		{QualifiedName: "b.G", Kind: graph.NodeKindFunction, Name: "G", FilePath: "/ns-b-only/file.go", StartLine: 1, EndLine: 2, Language: "go"},
	})
...
type failingRefreshMaintenance struct{ err error }

func (m failingRefreshMaintenance) RefreshDocuments(context.Context) (int, error) {
	return 0, m.err
}

func (m failingRefreshMaintenance) RebuildIndex(context.Context) error { return nil }
...
`)
	...
	if err := testDBFor(deps).Find(&docs).Error; err != nil {
...
`)
	...
	var count int64
	if err := testDBFor(deps).Model(&graph.SearchDocument{}).Count(&count).Error; err != nil {
