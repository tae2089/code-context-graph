...
func setupPromptTestDeps(t *testing.T) (*Deps, *gorm.DB) {
	...
	deps := groupedTestDeps(st, db, sb, &simpleGoParser{}, nil)
	registerTestDeps(t, deps, db, sb)
	return deps, db
}
...
func TestReviewChanges_ReturnsRiskEntries(t *testing.T) {
	...
	db.Create(&graph.Node{QualifiedName: "pkg.FuncA", Kind: graph.NodeKindFunction, Name: "FuncA", FilePath: "a.go", StartLine: 1, EndLine: 20, Language: "go"})
	db.Create(&graph.Node{QualifiedName: "pkg.FuncB", Kind: graph.NodeKindFunction, Name: "FuncB", FilePath: "b.go", StartLine: 1, EndLine: 10, Language: "go"})
...
func TestReviewChanges_DefaultBase(t *testing.T) {
	...
	db.Create(&graph.Node{QualifiedName: "pkg.Fn", Kind: graph.NodeKindFunction, Name: "Fn", FilePath: "fn.go", StartLine: 1, EndLine: 5, Language: "go"})
...
func TestReviewChanges_RespectsSmallerLimitArgument(t *testing.T) {
	...
	for i := 1; i <= 5; i++ {
		...
		db.Create(&graph.Node{QualifiedName: fmt.Sprintf("pkg.Small%02d", i), Kind: graph.NodeKindFunction, Name: fmt.Sprintf("Small%02d", i), FilePath: filePath, StartLine: 1, EndLine: 20, Language: "go"})
		changedFiles = append(changedFiles, filePath)
		hunks = append(hunks, changes.Hunk{FilePath: filePath, StartLine: 1, EndLine: 20})
	}

	setTestChangesClient(deps, &mockGitClient{changedFiles: changedFiles, hunks: hunks})
	repoRoot := t.TempDir()
	deps.Runtime.RepoRoot = repoRoot
...
func TestDebugIssue_ReturnsSearchResults(t *testing.T) {
	...
	db.Create(&graph.Node{QualifiedName: "pkg.HandleLogin", Kind: graph.NodeKindFunction, Name: "HandleLogin", FilePath: "login.go", StartLine: 1, EndLine: 10, Language: "go"})
	var node graph.Node
	db.First(&node, "qualified_name = ?", "pkg.HandleLogin")

	db.Create(&graph.SearchDocument{NodeID: node.ID, Content: "HandleLogin login authentication handler", Language: "go"})
...
func TestDebugIssue_IncludesCallGraph(t *testing.T) {
	...
	n1 := graph.Node{QualifiedName: "pkg.Caller", Kind: graph.NodeKindFunction, Name: "Caller", FilePath: "a.go", StartLine: 1, EndLine: 5, Language: "go"}
	n2 := graph.Node{QualifiedName: "pkg.Target", Kind: graph.NodeKindFunction, Name: "Target", FilePath: "b.go", StartLine: 1, EndLine: 5, Language: "go"}
	db.Create(&n1)
	db.Create(&n2)
	db.Create(&graph.Edge{FromNodeID: n1.ID, ToNodeID: n2.ID, Kind: graph.EdgeKindCalls, Fingerprint: "calls-c-t"})

	db.Create(&graph.SearchDocument{NodeID: n2.ID, Content: "Target function handler", Language: "go"})
	testSearchBackendFor(deps).Rebuild(ctx, testDBFor(deps))
...
func TestDebugIssue_TruncatesPerNodeCallGraph(t *testing.T) {
	...
	target := graph.Node{QualifiedName: "pkg.Target", Kind: graph.NodeKindFunction, Name: "Target", FilePath: "target.go", StartLine: 1, EndLine: 10, Language: "go"}
	db.Create(&target)
	db.Create(&graph.SearchDocument{NodeID: target.ID, Content: "Target issue handler", Language: "go"})
	...
	for i := 1; i <= 12; i++ {
		queryMock.result = append(queryMock.result, graph.Node{QualifiedName: fmt.Sprintf("pkg.Neighbor%02d", i)})
	}
	deps.Graph.Query = queryMock
...
func TestOnboardDeveloper_ReturnsStats(t *testing.T) {
	...
	db.Create(&graph.Node{QualifiedName: "pkg.A", Kind: graph.NodeKindFunction, Name: "A", FilePath: "a.go", StartLine: 1, EndLine: 5, Language: "go"})
	db.Create(&graph.Node{QualifiedName: "pkg.B", Kind: graph.NodeKindFunction, Name: "B", FilePath: "b.go", StartLine: 1, EndLine: 5, Language: "python"})
	db.Create(&graph.Edge{FromNodeID: 1, ToNodeID: 2, Kind: graph.EdgeKindCalls, Fingerprint: "e1"})
...
func TestOnboardDeveloper_TruncatesLanguageSection(t *testing.T) {
	...
	for i := 1; i <= 12; i++ {
		...
		db.Create(&graph.Node{QualifiedName: fmt.Sprintf("pkg.Node%02d", i), Kind: graph.NodeKindFunction, Name: fmt.Sprintf("Node%02d", i), FilePath: fmt.Sprintf("node%02d.go", i), StartLine: 1, EndLine: 5, Language: lang})
	}
...
func TestPreMergeCheck_ReturnsRisk(t *testing.T) {
	...
	db.Create(&graph.Node{QualifiedName: "pkg.Handler", Kind: graph.NodeKindFunction, Name: "Handler", FilePath: "handler.go", StartLine: 1, EndLine: 20, Language: "go"})
...
func TestPreMergeCheck_TruncatesRiskSection(t *testing.T) {
	...
	for i := 1; i <= 22; i++ {
		...
		db.Create(&graph.Node{QualifiedName: fmt.Sprintf("pkg.Merge%02d", i), Kind: graph.NodeKindFunction, Name: fmt.Sprintf("Merge%02d", i), FilePath: filePath, StartLine: 1, EndLine: 60, Language: "go"})
		changedFiles = append(changedFiles, filePath)
...
func TestReviewChanges_AllowsNamespaceRootWhenRepoRootAlsoConfigured(t *testing.T) {
	deps, _ := setupPromptTestDeps(t)
	setTestChangesClient(deps, &mockGitClient{changedFiles: []string{}})
	deps.Runtime.RepoRoot = t.TempDir()
	deps.Runtime.NamespaceRoot = t.TempDir()
	namespaceRoot := t.TempDir()
	deps.Runtime.NamespaceRoot = namespaceRoot
