...
func TestGetNode_CacheHit(t *testing.T) {
	...
	h := &handlers{deps: &Deps{Graph: GraphToolsDeps{Store: st}}, cache: cache}
	...
	if err := db.Unscoped().Delete(&graph.Node{}, node.ID).Error; err != nil {
		t.Fatal(err)
	}
...
func TestGetNode_NoCache(t *testing.T) {
	...
	h := &handlers{deps: &Deps{Graph: GraphToolsDeps{Store: st}}, cache: nil}
