...
func TestGetMinimalContext_WithNodesAndEdges(t *testing.T) {
	...
	testDBFor(deps).Create(&graph.Node{QualifiedName: "pkg.FuncA", Kind: graph.NodeKindFunction, Name: "FuncA", FilePath: "a.go", StartLine: 1, EndLine: 5, Language: "go"})
	testDBFor(deps).Create(&graph.Node{QualifiedName: "pkg.FuncB", Kind: graph.NodeKindFunction, Name: "FuncB", FilePath: "b.go", StartLine: 1, EndLine: 10, Language: "go"})
	testDBFor(deps).Create(&graph.Node{QualifiedName: "pkg.FuncC", Kind: graph.NodeKindFunction, Name: "FuncC", FilePath: "a.go", StartLine: 7, EndLine: 15, Language: "go"})

	testDBFor(deps).Create(&graph.Edge{FromNodeID: 1, ToNodeID: 2, Kind: graph.EdgeKindCalls, Fingerprint: "e1"})
	testDBFor(deps).Create(&graph.Edge{FromNodeID: 2, ToNodeID: 3, Kind: graph.EdgeKindCalls, Fingerprint: "e2"})
...
func TestGetMinimalContext_CommunitiesAndFlows(t *testing.T) {
	...
	for _, c := range []struct {
		...
		comm := graph.Community{Key: c.label, Label: c.label, Strategy: "directory"}
		testDBFor(deps).Create(&comm)
		for j := 0; j < c.size; j++ {
			testDBFor(deps).Create(&graph.CommunityMembership{CommunityID: comm.ID, NodeID: nodeIDs[nodeIdx%len(nodeIDs)]})
			nodeIdx++
		}
	}
	...
	for _, f := range []struct {
		...
		flow := graph.Flow{Name: f.name}
		testDBFor(deps).Create(&flow)
		for j := 0; j < f.size; j++ {
			testDBFor(deps).Create(&graph.FlowMembership{FlowID: flow.ID, NodeID: nodeIDs[j%len(nodeIDs)], Ordinal: j})
		}
	}
...
func TestGetMinimalContext_CommunitiesAndFlows_StableTieBreakByID(t *testing.T) {
	...
	for i := 1; i <= 6; i++ {
		testDBFor(deps).Create(&graph.Node{
	...
	for _, label := range []string{"comm-a", "comm-b", "comm-c", "comm-d"} {
		comm := graph.Community{Label: label, Key: label, Strategy: "directory"}
		testDBFor(deps).Create(&comm)
		communities = append(communities, comm)
	}
	for i, comm := range communities {
		testDBFor(deps).Create(&graph.CommunityMembership{CommunityID: comm.ID, NodeID: uint(i + 1)})
	}

	var flows []graph.Flow
	for _, name := range []string{"flow-a", "flow-b", "flow-c", "flow-d"} {
		flow := graph.Flow{Name: name}
		testDBFor(deps).Create(&flow)
		flows = append(flows, flow)
	}
	for i, flow := range flows {
		testDBFor(deps).Create(&graph.FlowMembership{FlowID: flow.ID, NodeID: uint(i + 1), Ordinal: 0})
	}
...
func TestGetMinimalContext_RespectsFlowNamespace(t *testing.T) {
	...
	if err := testGraphStoreFor(deps).UpsertNodes(ctx, []graph.Node{{QualifiedName: "pkg.N1", Kind: graph.NodeKindFunction, Name: "N1", FilePath: "a.go", StartLine: 1, EndLine: 5, Language: "go"}}); err != nil {
		t.Fatal(err)
	}
	node, _ := testGraphStoreFor(deps).GetNode(ctx, "pkg.N1")
	alpha := graph.Flow{Namespace: "alpha", Name: "alpha_flow"}
	beta := graph.Flow{Namespace: "beta", Name: "beta_flow"}
	testDBFor(deps).Create(&alpha)
	testDBFor(deps).Create(&beta)
	testDBFor(deps).Create(&graph.FlowMembership{Namespace: "alpha", FlowID: alpha.ID, NodeID: node.ID, Ordinal: 0})
	testDBFor(deps).Create(&graph.FlowMembership{Namespace: "beta", FlowID: beta.ID, NodeID: node.ID, Ordinal: 0})
...
func TestGetMinimalContext_WithRepoRoot(t *testing.T) {
	...
	testDBFor(deps).Create(&graph.Node{QualifiedName: "pkg.Login", Kind: graph.NodeKindFunction, Name: "Login", FilePath: "auth.go", StartLine: 1, EndLine: 20, Language: "go"})
	testDBFor(deps).Create(&graph.Node{QualifiedName: "pkg.Logout", Kind: graph.NodeKindFunction, Name: "Logout", FilePath: "auth.go", StartLine: 22, EndLine: 30, Language: "go"})
	testDBFor(deps).Create(&graph.Edge{FromNodeID: 1, ToNodeID: 2, Kind: graph.EdgeKindCalls, Fingerprint: "e1"})
...
func TestGetMinimalContext_RejectsRepoRootOutsideConfiguredRoot(t *testing.T) {
	deps := setupTestDeps(t)
	setTestChangesClient(deps, &contextMockGitClient{})
