...
func setupE2EDeps(t *testing.T) *Deps {
	...
	return registerTestDeps(t, groupedTestDeps(st, db, sb, goParser, nil), db, sb)
}
...
`)
	...
	if node["kind"] != string(graph.NodeKindFunction) {
...
`)
	...
	apiNode, err := testGraphStoreFor(deps).GetNode(ctx, filepath.Join(apiDir, "handler.go"))
	if err != nil || apiNode == nil {
		apiNode, err = testGraphStoreFor(deps).GetNode(ctx, "api.Handler")
	}
	if err != nil || apiNode == nil {
		t.Fatalf("expected api.Handler after scoped rebuild, err=%v", err)
	}
	otherNode, err := testGraphStoreFor(deps).GetNode(ctx, "other.Other")
	if err != nil {
...
`)
	...
	node, err := testGraphStoreFor(deps).GetNode(ctx, "auth.AuthenticateUser")
	if err != nil || node == nil {
	...
	testGraphStoreFor(deps).UpsertAnnotation(ctx, &graph.Annotation{
		...
		Tags: []graph.DocTag{
			{Kind: graph.TagParam, Name: "username", Value: "the user login ID", Ordinal: 0},
			{Kind: graph.TagParam, Name: "password", Value: "plaintext password", Ordinal: 1},
			{Kind: graph.TagReturn, Value: "JWT token on success", Ordinal: 0},
			{Kind: graph.TagIntent, Value: "verify credentials before session creation", Ordinal: 0},
			{Kind: graph.TagDomainRule, Value: "lock account after 5 consecutive failures", Ordinal: 0},
		},
	})
...
func TestE2E_IncrementalReparse(t *testing.T) {
	...
	syncer := incremental.New(testGraphStoreFor(deps), deps.Build.Walkers[".go"])
...
`
	...
	// Verify node exists
	node, err := testGraphStoreFor(deps).GetNode(ctx, "calc.Add")
	if err != nil || node == nil {
...
`
	...
	newNode, err := testGraphStoreFor(deps).GetNode(ctx, "calc.Subtract")
...
`)
	...
	handleReq, _ := testGraphStoreFor(deps).GetNode(ctx, "service.HandleRequest")
	validate, _ := testGraphStoreFor(deps).GetNode(ctx, "service.ValidateInput")
	process, _ := testGraphStoreFor(deps).GetNode(ctx, "service.ProcessData")
	saveToDB, _ := testGraphStoreFor(deps).GetNode(ctx, "service.SaveToDatabase")
	...
	testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: handleReq.ID, ToNodeID: validate.ID, Kind: graph.EdgeKindCalls, FilePath: "service.go", Line: 4, Fingerprint: "calls:hr:vi"},
		{FromNodeID: handleReq.ID, ToNodeID: process.ID, Kind: graph.EdgeKindCalls, FilePath: "service.go", Line: 5, Fingerprint: "calls:hr:pd"},
		{FromNodeID: process.ID, ToNodeID: saveToDB.ID, Kind: graph.EdgeKindCalls, FilePath: "service.go", Line: 12, Fingerprint: "calls:pd:sd"},
	})
...
`)
	...
	nodesInFile, err := testGraphStoreFor(deps).GetNodesByFile(ctx, "repository.go")
	if err != nil {
		t.Fatal(err)
	}
	...
	results := decodeSearchResults(t, getTextContent(searchResult))
	if len(results) < 2 {
...
`)
	...
	nodeMain, _ := testGraphStoreFor(deps).GetNode(ctx, "caller.Main")
	nodeHelper, _ := testGraphStoreFor(deps).GetNode(ctx, "caller.Helper")
	if nodeMain == nil || nodeHelper == nil {
		t.Fatal("expected both nodes to exist after build")
	}
	testGraphStoreFor(deps).UpsertEdges(ctx, []graph.Edge{
		{FromNodeID: nodeMain.ID, ToNodeID: nodeHelper.ID, Kind: graph.EdgeKindCalls, Fingerprint: "calls-main-helper"},
	})

	// Step 3: Setup QueryService and query_graph
	queryService := query.New(graphgorm.New(testDBFor(deps)))
	deps.Graph.Query = queryService
