...
type failingGraphViewRepository struct {
	wiki.Repository
	err error
}

func (r failingGraphViewRepository) GraphView(context.Context, int, []graph.EdgeKind) (wiki.GraphView, error) {
	return wiki.GraphView{}, r.err
}

var wikiTestDatabases sync.Map
...
func newTestServer(t *testing.T) *Server {
	...
	if err := db.AutoMigrate(&graph.Node{}, &graph.Edge{}, &graph.Annotation{}, &graph.DocTag{}); err != nil {
		t.Fatalf("migrate nodes: %v", err)
	}
	fileNode := graph.Node{Namespace: requestctx.DefaultNamespace, QualifiedName: "main.go", Kind: graph.NodeKindFile, Name: "main.go", FilePath: "main.go", StartLine: 1, EndLine: 20, Language: "go"}
	if err := db.Create(&fileNode).Error; err != nil {
		t.Fatalf("create file node: %v", err)
	}
	funcNode := graph.Node{Namespace: requestctx.DefaultNamespace, QualifiedName: "main.Run", Kind: graph.NodeKindFunction, Name: "Run", FilePath: "main.go", StartLine: 3, EndLine: 8, Language: "go"}
	if err := db.Create(&funcNode).Error; err != nil {
		t.Fatalf("create function node: %v", err)
	}
	if err := db.Create(&graph.Edge{Namespace: requestctx.DefaultNamespace, FromNodeID: fileNode.ID, ToNodeID: funcNode.ID, Kind: graph.EdgeKindContains, FilePath: "main.go", Line: 3, Fingerprint: "default-contains-run"}).Error; err != nil {
		t.Fatalf("create edge: %v", err)
	}
	if err := db.Create(&graph.Node{Namespace: "sample-go", QualifiedName: "main.Run", Kind: graph.NodeKindFunction, Name: "Run", FilePath: "main.go"}).Error; err != nil {
		t.Fatalf("create node: %v", err)
	}
	authNode := graph.Node{Namespace: "auth-svc", QualifiedName: "auth.ValidateToken", Kind: graph.NodeKindFunction, Name: "ValidateToken", FilePath: "internal/auth/token.go", StartLine: 10, EndLine: 24, Language: "go"}
...
func TestAPI_NamespacesMergesDatabaseAndIndex(t *testing.T) {
	...
	if !contains(got.Namespaces, requestctx.DefaultNamespace) || !contains(got.Namespaces, "sample-go") {
...
func TestAPI_TreeFallsBackToDBWhenWikiIndexMissing(t *testing.T) {
	...
	var got struct {
		...
		Root      *wiki.TreeNode `json:"root"`
	}
	if err := json.Unmarshal(rec.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode tree fallback: %v", err)
	}
	if got.Namespace != requestctx.DefaultNamespace || got.Root == nil {
...
func TestAPI_GraphReturnsNodesAndEdges(t *testing.T) {
	...
	if got.Edges[0].Kind != string(graph.EdgeKindContains) {
		t.Fatalf("edge kind = %q", got.Edges[0].Kind)
	}
}

func TestAPI_GraphPreservesRepositoryFailureStage(t *testing.T) {
	tests := []struct {
		stage wiki.GraphViewStage
		want  string
	}{
		{stage: wiki.GraphViewStageCountNodes, want: "count graph nodes"},
		{stage: wiki.GraphViewStageListNodes, want: "list graph nodes"},
		{stage: wiki.GraphViewStageListEdges, want: "list graph edges"},
	}
	for _, tt := range tests {
		t.Run(string(tt.stage), func(t *testing.T) {
			srv := newTestServer(t)
			srv.repository = failingGraphViewRepository{
				Repository: srv.repository,
				err:        &wiki.GraphViewError{Stage: tt.stage, Err: errors.New("database unavailable")},
			}
			rec := httptest.NewRecorder()
			req := httptest.NewRequest(http.MethodGet, "/wiki/api/graph?namespace=default", nil)
			srv.APIHandler().ServeHTTP(rec, req)
			...
			if body["error"] != tt.want {
				t.Fatalf("error = %v, want %q", body["error"], tt.want)
			}
			if body["detail"] != "database unavailable" {
...
func findTreeNode(root *wiki.TreeNode, id string) *wiki.TreeNode {
	if root == nil {
		return nil
	}
	if root.ID == id {
