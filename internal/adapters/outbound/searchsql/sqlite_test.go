...
func TestSQLiteFTS_Query_PromotesExactNameMatch(t *testing.T) {
	...
	nodes := []graph.Node{
		{Namespace: requestctx.DefaultNamespace, QualifiedName: "cpp.UserService.getUser", Kind: graph.NodeKindFunction, Name: "getUser", FilePath: "cpp/sample.cpp", StartLine: 1, EndLine: 2, Language: "cpp"},
		{Namespace: requestctx.DefaultNamespace, QualifiedName: "go.UserService.GetUser", Kind: graph.NodeKindFunction, Name: "GetUser", FilePath: "go/sample.go", StartLine: 1, EndLine: 2, Language: "go"},
		{Namespace: requestctx.DefaultNamespace, QualifiedName: "java.UserService.getUser", Kind: graph.NodeKindFunction, Name: "getUser", FilePath: "java/Sample.java", StartLine: 1, EndLine: 2, Language: "java"},
	}
	for i := range nodes {
	...
	docs := []graph.SearchDocument{
		{Namespace: requestctx.DefaultNamespace, NodeID: nodes[0].ID, Content: "getUser cpp sample cpp", Language: "cpp"},
		{Namespace: requestctx.DefaultNamespace, NodeID: nodes[1].ID, Content: "GetUser go sample go", Language: "go"},
		{Namespace: requestctx.DefaultNamespace, NodeID: nodes[2].ID, Content: "getUser java sample java", Language: "java"},
	}
	for i := range docs {
...
func TestSQLiteFTS_RebuildNodes_RefreshesOnlyScopedNodeRows(t *testing.T) {
	...
	if err := db.Exec("INSERT INTO search_fts(node_id, content, language, namespace) VALUES (?, ?, ?, ?), (?, ?, ?, ?), (?, ?, ?, ?)", changed.ID, "stale changed", "go", requestctx.DefaultNamespace, untouched.ID, "keep untouched", "go", requestctx.DefaultNamespace, foreign.ID, "keep foreign", "go", "ns-b").Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Raw("SELECT content FROM search_fts WHERE node_id = ? AND namespace = ?", changed.ID, requestctx.DefaultNamespace).Scan(&changedContent).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Raw("SELECT content FROM search_fts WHERE node_id = ? AND namespace = ?", untouched.ID, requestctx.DefaultNamespace).Scan(&untouchedContent).Error; err != nil {
		t.Fatal(err)
	}
...
func TestSQLiteFTS_RebuildNodes_EmptyScopeIsNoOp(t *testing.T) {
	...
	if err := db.Exec("INSERT INTO search_fts(node_id, content, language, namespace) VALUES (?, ?, ?, ?)", 1, "keep", "go", requestctx.DefaultNamespace).Error; err != nil {
		t.Fatal(err)
	}
...
func TestSQLiteFTS_Query_EmptyNamespace_IsLiteralFilter(t *testing.T) {
	...
	if err := db.Create(&graph.SearchDocument{Namespace: requestctx.DefaultNamespace, NodeID: defaultNode.ID, Content: "sharedterm default", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&graph.SearchDocument{Namespace: "ns-b", NodeID: otherNode.ID, Content: "sharedterm other", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := backend.Rebuild(context.Background(), db); err != nil {
		t.Fatal(err)
...
func TestSQLiteFTS_Rebuild_EmptyNamespace_PreservesOtherRows(t *testing.T) {
	...
	defaultNode := graph.Node{Namespace: requestctx.DefaultNamespace, QualifiedName: "pkg.Default", Kind: graph.NodeKindFunction, Name: "Default", FilePath: "default.go", StartLine: 1, EndLine: 1, Language: "go"}
	otherNode := graph.Node{Namespace: "ns-b", QualifiedName: "pkg.Other", Kind: graph.NodeKindFunction, Name: "Other", FilePath: "other.go", StartLine: 1, EndLine: 1, Language: "go"}
	if err := db.Create(&defaultNode).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&otherNode).Error; err != nil {
	...
	if err := db.Create(&graph.SearchDocument{Namespace: requestctx.DefaultNamespace, NodeID: defaultNode.ID, Content: "default term", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&graph.SearchDocument{Namespace: "ns-b", NodeID: otherNode.ID, Content: "other term", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := backend.Rebuild(context.Background(), db); err != nil {
		t.Fatal(err)
	...
	if err := db.Raw("SELECT count(*) FROM search_fts WHERE namespace = ?", requestctx.DefaultNamespace).Scan(&defaultCount).Error; err != nil {
		t.Fatal(err)
	}
...
func TestSQLiteFTS_Rebuild_BatchesLargeDatasets(t *testing.T) {
	...
	for i := 0; i < sqliteFTSRebuildBatchSize+25; i++ {
		node := graph.Node{QualifiedName: fmt.Sprintf("pkg.Node%d", i), Kind: graph.NodeKindFunction, Name: fmt.Sprintf("Node%d", i), FilePath: fmt.Sprintf("file%d.go", i), StartLine: i + 1, EndLine: i + 1, Language: "go"}
		if err := db.Create(&node).Error; err != nil {
			t.Fatal(err)
		}
		doc := graph.SearchDocument{NodeID: node.ID, Content: fmt.Sprintf("term%d batched content", i), Language: "go"}
...
func TestSQLiteFTS_Rebuild_RollsBackDeleteOnInsertFailure(t *testing.T) {
	...
	seedNode := graph.Node{QualifiedName: "pkg.Seed", Kind: graph.NodeKindFunction, Name: "Seed", FilePath: "seed.go", StartLine: 1, EndLine: 1, Language: "go"}
	if err := db.Create(&seedNode).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&graph.SearchDocument{NodeID: seedNode.ID, Content: "seed term", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := backend.Rebuild(context.Background(), db); err != nil {
		t.Fatal(err)
	...
	backend.batchInserter = func(ctx context.Context, tx *gorm.DB, tableName string, docs []graph.SearchDocument) error {
...
func TestSQLiteFTS_Rebuild_NamespaceRollbackPreservesOtherRows(t *testing.T) {
	...
	nodeA := graph.Node{Namespace: "ns-a", QualifiedName: "pkg.A", Kind: graph.NodeKindFunction, Name: "A", FilePath: "a.go", StartLine: 1, EndLine: 1, Language: "go"}
	nodeB := graph.Node{Namespace: "ns-b", QualifiedName: "pkg.B", Kind: graph.NodeKindFunction, Name: "B", FilePath: "b.go", StartLine: 1, EndLine: 1, Language: "go"}
	...
	if err := db.Create(&graph.SearchDocument{Namespace: "ns-a", NodeID: nodeA.ID, Content: "term a", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&graph.SearchDocument{Namespace: "ns-b", NodeID: nodeB.ID, Content: "term b", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := backend.Rebuild(requestctx.WithNamespace(context.Background(), "ns-a"), db); err != nil {
		t.Fatal(err)
	...
	err := backend.Rebuild(requestctx.WithNamespace(context.Background(), "ns-a"), db)
	if err == nil {
...
func TestSQLiteFTS_Rebuild_RollsBackWithOuterTransaction(t *testing.T) {
	...
	seedNode := graph.Node{QualifiedName: "pkg.Seed", Kind: graph.NodeKindFunction, Name: "Seed", FilePath: "seed.go", StartLine: 1, EndLine: 1, Language: "go"}
	newNode := graph.Node{QualifiedName: "pkg.New", Kind: graph.NodeKindFunction, Name: "New", FilePath: "new.go", StartLine: 1, EndLine: 1, Language: "go"}
	if err := db.Create(&seedNode).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&graph.SearchDocument{NodeID: seedNode.ID, Content: "seedterm", Language: "go"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := backend.Rebuild(context.Background(), db); err != nil {
		t.Fatal(err)
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("node_id = ?", seedNode.ID).Delete(&graph.SearchDocument{}).Error; err != nil {
			return err
		}
		if err := tx.Create(&graph.SearchDocument{NodeID: newNode.ID, Content: "newterm", Language: "go"}).Error; err != nil {
			return err
		}
