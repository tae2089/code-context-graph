...
// Checkout binds repository root, locking, and optional transport auth to the app port.
// @intent expose one locked checkout capability while retaining go-git types inside the adapter.
...
func NewCheckout(root string, locker *RepoLocker, auth transport.AuthMethod) *Checkout {
	return &Checkout{Root: root, Locker: locker, Auth: auth}
}

// Sync clones or resets the requested branch under per-repository locking.
// @intent make the requested namespace checkout match the admitted remote branch before graph update.
// @sideEffect mutates the namespace checkout below the configured repository root.
func (c *Checkout) Sync(ctx context.Context, req reposync.CheckoutRequest) (string, error) {
	if err := CloneOrPullBranchLocked(ctx, c.Locker, req.CloneURL, c.Root, req.RepoFullName, req.Namespace, req.Branch, c.Auth); err != nil {
		return "", err
	}
