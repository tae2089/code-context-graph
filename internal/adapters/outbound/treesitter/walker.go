...
func (w *Walker) executeQueries(root *sitter.Node, content []byte, filePath string, pkgName string, importPackages map[string]string, nodes []graph.Node, edges []graph.Edge) ([]graph.Node, []graph.Edge, string, []interfaceInfo, error) {
	...
	for {
		...
		if defNode != nil && nameNode != nil {
			...
			if idx, exists := nodeIndex[key]; exists {
				...
				if nodes[idx].Kind == graph.NodeKindType && kind == graph.NodeKindClass {
					nodes[idx].Kind = kind
				}
			...
			if shouldEnrich {
				...
				for _, traitName := range implementedTypes {
					edges = appendUniqueEdges(edges, edgeSeen, graph.Edge{
...
func (w *Walker) mapDefTypeToNodeKind(defType string, name string) graph.NodeKind {
	switch defType {
	case "class":
		return graph.NodeKindClass
	case "interface", "type":
		return graph.NodeKindType
	default:
		if isTestName(name, w.spec.TestPrefix) {
			return graph.NodeKindTest
		}
...
func (w *Walker) resolveTestedBy(nodes []graph.Node, edges *[]graph.Edge, filePath string) {
	if w.spec.TestPrefix == "" {
		return
	}

	testNodes := make(map[string]graph.Node)

	for _, n := range nodes {
		if n.Kind == graph.NodeKindTest {
