...
import (
	...
	"github.com/tae2089/code-context-graph/internal/domain/annotation"
	"github.com/tae2089/code-context-graph/internal/domain/graph"
)

// fixtureDir은 binding_gap fixture 디렉토리의 절대 경로를 반환합니다.
func fixtureDir(lang string) string {
	// 이 파일은 internal/adapters/outbound/treesitter/ 에 위치.
	...
	// internal/adapters/outbound/treesitter → 루트로 4단계 올라감
...
// binderFromWalkerComments는 Walker의 CommentBlock을 binding.CommentBlock으로 변환합니다.
// (treesitter.CommentBlock과 binding.CommentBlock은 별도 타입이므로 변환이 필요함)
func binderFromWalkerComments(wcs []CommentBlock) []binding.CommentBlock {
	result := make([]binding.CommentBlock, len(wcs))
	for i, wc := range wcs {
		result[i] = binding.CommentBlock{
...
func logNodeInfo(t *testing.T, nodes []graph.Node) {
	t.Helper()
	for _, n := range nodes {
...
func TestWalkerBinder_PythonDecorator_CurrentlyFailsBinding(t *testing.T) {
	...
	var getUserBinding *binding.Binding
...
func TestWalkerBinder_JavaAnnotation_CurrentlyFailsBinding(t *testing.T) {
	...
	var userServiceNode *graph.Node
	for i := range nodes {
	...
	var userServiceBinding *binding.Binding
	for i := range bindings {
...
func TestWalkerBinder_RustAttribute_CurrentlyFailsBinding(t *testing.T) {
	...
	for i := range nodes {
		if nodes[i].Name == "main" && nodes[i].Kind == graph.NodeKindFunction {
			mainNode = &nodes[i]
			break
		}
	}
	...
	var mainBinding *binding.Binding
	for i := range bindings {
...
func TestWalkerBinder_CAttribute_CurrentlyFailsBinding(t *testing.T) {
	...
	for i := range nodes {
		if nodes[i].Name == "add" && nodes[i].Kind == graph.NodeKindFunction {
			addNode = &nodes[i]
			break
		}
	}
	...
	var addBinding *binding.Binding
	for i := range bindings {
...
func TestWalkerBinder_GapDiagnosis_AllLanguages(t *testing.T) {
	type langCase struct {
		...
		filename   string
		symName    string
		symKind    graph.NodeKind
		fixtureExt string
	}

	cases := []langCase{
		{PythonSpec, "python", "decorator_gap.py", "get_user", graph.NodeKindFunction, "py"},
		{JavaSpec, "java", "AnnotationGap.java", "UserService", graph.NodeKindClass, "java"},
		{RustSpec, "rust", "attribute_gap.rs", "main", graph.NodeKindFunction, "rs"},
		{CSpec, "c", "attribute_gap.c", "add", graph.NodeKindFunction, "c"},
	}
