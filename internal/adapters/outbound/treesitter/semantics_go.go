...
func goStructuralImplements(nodes []graph.Node, ifaces []PackageInterfaceInfo, scope string) []graph.Edge {
...
func goAssertionImplements(root *sitter.Node, content []byte, filePath string, repoPackages map[string]string) []graph.Edge {
	if root == nil {
		return nil
	}
...
func extractGoEmbeddings(structNode *sitter.Node, content []byte, filePath, structName string) []graph.Edge {
	var edges []graph.Edge
			...
			for j := 0; j < int(child.ChildCount()); j++ {
				...
				if !hasFieldName && typeName != "" {
					...
					edges = append(edges, graph.Edge{
						...
						FilePath:    filePath,
						Line:        line,
						Fingerprint: graph.BuildInheritsFingerprintV2(filePath, structName, typeName),
					})
				}
			}
		}
