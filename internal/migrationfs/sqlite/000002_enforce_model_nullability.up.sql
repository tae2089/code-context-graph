...
SELECT id, namespace, COALESCE(qualified_name, ''), COALESCE(kind, ''), COALESCE(name, ''), COALESCE(file_path, ''), COALESCE(start_line, 0), COALESCE(end_line, 0), hash, language, created_at, updated_at
FROM nodes;
DROP TABLE nodes;
...
SELECT id, namespace, from_node_id, to_node_id, COALESCE(kind, ''), file_path, line, COALESCE(fingerprint, ''), created_at
FROM edges;
DROP TABLE edges;
...
SELECT id, namespace, COALESCE(key, ''), label, strategy, description, created_at, updated_at
FROM communities;
...
SELECT id, COALESCE(community_id, 0), COALESCE(node_id, 0), created_at
...
SELECT id, namespace, COALESCE(name, ''), description, created_at
FROM flows;
DROP TABLE flows;
...
SELECT id, namespace, COALESCE(flow_id, 0), COALESCE(node_id, 0), ordinal
