...
// @sideEffect graph store와 service 레벨 테이블, 검색 백엔드를 하나의 DB 트랜잭션으로 정리한다.
...
func (p *NamespacePurger) Purge(ctx context.Context) error {
	...
	txStore, ok := p.store.(transactionalDBStore)
	if !ok {
		return trace.New("namespace purge requires graph store with DB transaction handle")
	}

	return txStore.WithTxDB(ctx, func(graphStore store.GraphStore, tx *gorm.DB) error {
		if err := graphStore.DeleteGraph(ctx); err != nil {
			return trace.Wrap(err, "delete namespace graph")
		}
		return p.purgeServiceState(ctx, tx)
	})
}
