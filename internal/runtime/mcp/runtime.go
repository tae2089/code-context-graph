...
import (
	...
	"github.com/tae2089/code-context-graph/internal/adapters/outbound/gitexec"
...
// Components is the runtime-owned application and adapter set required to assemble MCP.
// @intent share one MCP assembly path without making the MCP runtime import its parent composition package.
...
func New(components Components, opts Options) (*Instance, error) {
	components.Logger.Info("starting code-context-graph MCP runtime")
	...
	if !opts.NoCache && opts.CacheTTL > 0 {
		...
		components.Logger.Info("MCP cache enabled", "ttl", opts.CacheTTL)
	}

	mcpWalkers := make(map[string]mcp.Parser, len(components.Walkers))
	for ext, w := range components.Walkers {
		mcpWalkers[ext] = w
	}

	mcpDeps := &mcp.Deps{
		Build: mcp.BuildToolsDeps{
			Store: components.Store, Walkers: mcpWalkers, UnitOfWork: components.UnitOfWork,
			Search: components.Search, Maintenance: components.SearchMaintenance,
			FlowBuilder: flows.NewBuilder(components.Store), Incremental: components.Syncer,
		},
		...
		Runtime: mcp.RuntimeToolsDeps{
			...
			RepoRoot: opts.RepoRoot, MaxFileBytes: opts.MaxFileBytes,
...
func RunStdio(components Components, opts Options) error {
	inst, err := New(components, opts)
	if err != nil {
		return err
	}
	defer inst.Close()

	components.Logger.Info("serving MCP over stdio")
	...
	case <-ctx.Done():
		components.Logger.Info("received signal, shutting down stdio MCP server")
	}
	return nil
}
