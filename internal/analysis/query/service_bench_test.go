...
func setupQueryBenchDB(b *testing.B, includeFallback bool) *gorm.DB {
	...
	nodes := make([]model.Node, benchNodeCount)
	...
	edges := make([]model.Edge, 0, benchNodeCount*benchEdgesPerNode)
	for i := 0; i < benchNodeCount; i++ {
...
func BenchmarkQueryService_FindExactNameMatches(b *testing.B) {
	...
	if err := db.Model(&model.Node{}).Where("id <= ?", duplicated).Update("name", "CommonName").Error; err != nil {
